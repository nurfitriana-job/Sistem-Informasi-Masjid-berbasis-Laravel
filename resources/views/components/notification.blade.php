...
<x-filament-notifications::notification :notification="$notification" :x-transition:enter-start="
    ...
    " @class([
        ...
        \Filament\Support\get_color_css_variables($color, shades: [50, 400, 600], alias: 'notifications::notification') => !($isInline || $color === 'gray'),
    ])>
    <div @class([
        ...
        <div class="mt-0.5 grid flex-1">
            ...
            @if ($hasBody)
                <x-filament-notifications::body @class(['mt-1' => $hasTitle || $hasDate])>
                    <div x-data="{ body: @js(str($body)->sanitizeHtml()->toString()) }" x-html="body" x-init="Echo.private('App.Models.User.{{ auth()->user()->id }}').listen('.UpdateNotificationBody', (event) => { if (event.id == '{{ $notification->getId() }}') body = event.body });
